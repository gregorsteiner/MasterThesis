...
For the effect of heat exposure we utilize a different design. A binary treatment indicator is not-well suited to capture the effect of cumulative heat exposure, as heat exposure is continous. Instead, we rely on the two aforementioned measures: average maximum daily temperature and days above 30°C (the threshold of 30°C is somewhat arbitrary, but changing it slightly does not alter the results). These give rise to an interesting marginal interpretation: What is the effect of a 1°C hotter school year or of one additional day above 30°C on average test scores?
...
\end{align}
where $H_{i, t}$ is the measure of heat exposure at the county-schoolyear level. Since there are likely no confounding factors, a least squares estimate of $\beta$ can be interpreted causally.
